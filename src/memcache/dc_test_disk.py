...
# 作者: WangYuanbo
# --------------------------------------------------
# 用法 python dc_test_disk.py && sleep 180 && python dc_test_disk.py --randread
import argparse
import os
import sys
import threading
...
parser.add_argument('--randread', action="store_true", help='设置时表示randread，否则randwrite')
...
trouble_name = "read" if args.randread else "write"
...
time_spent = 180
rw_mode = "randread" if args.randread else "randwrite"
min_BW = 2
step = 2
max_BW = 40
...
print_list.extend([
    ...
    "rw_mode = {}".format(rw_mode),
    "min_BW = {}".format(min_BW),
    "step = {}".format(step),
    "max_BW = {}".format(max_BW),
    "fio_cmd = ssh node16 docker exec fiotest ./fio_test.sh {} {} {} {} {}".
    format(time_spent, rw_mode, min_BW, step, max_BW)
])
